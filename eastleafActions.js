(function (_4TellBoost, $, undefined) {
    ...
    if (_4TellBoost.CONFIG.SiteInfo.searchBoostEnable === true) {
        ...
        recWrapper.append(loadingGIF, closeButton, productListWrapper, divSelect, expandButton); 
        ...
        function doneTyping() {
            ...
            if (count > 2) {
                ...
                if (inputValue.length < 3) {
                    ...
                    //CUSTOM CODE TO REDIRECT BONSAI TO "view all bonsai trees"
                    if (inputValue.toLowerCase() === "bon" || inputValue === "bons" || inputValue === "bonsa" || inputValue === "bonsai") {
                        inputValue = "view all bonsai trees";
                    }
                    
                    //set up our variables 
                    var UserID = _4TellBoost.getCustomerID();
                    var table = 'catalog'; //We will always use catalog.
                    var term = $('.searchBoost4T').val(); //pillow or something
                    ...
                    if (_4TellBoost.UserData.session.searchToken === null || _4TellBoost.UserData.session.searchToken === undefined) {
                        //use search/start
                        var JsonPreString = {
                            "User": UserID,
                            "Table": "Catalog",
                        ...
                        $.ajax({
                            url: "//" + _4TellBoost.Service.address + "rest/Search/Start?clientAlias=" + _4TellBoost.SiteInfo.alias,
                            type: "POST",
                            xhrFields: {
                        ...
                        //use search/update
                        //build the searchterm string
                        //var dataParam = 'SearchTerm:"' + inputValue + '"';
                        // http://stage.4-tell.net/Boost2.0/rest/Search/Update?clientAlias={CLIENTALIAS}&token={TOKEN} 
                        var JsonPreString = {
                            "SearchTerm": inputValue,
                        };
                        var myJSON = JSON.stringify(JsonPreString);
                        myJSON = JSON.stringify(myJSON);
                        $.ajax({
                            url: "//" + _4TellBoost.Service.address + "rest/Search/Update?clientAlias=" + _4TellBoost.SiteInfo.alias + "&token=" + _4TellBoost.UserData.session.searchToken,
                            type: "POST",
                            xhrFields: {
                                withCredentials: false
                            },
                            ...
                            error: function () {
                                console.log('failure at search/update');
                                _4TellBoost.UserData.session.searchToken = null;
                                _4TellBoost.UserData.save();
                                doneTyping();
                            },
        ...
        function myGetResultsInterval() {
            var getResults = setInterval(function () {

                if (_4TellBoost.UserData.session.searchToken !== null || _4TellBoost.UserData.session.searchToken !== undefined) {

                    //stop looking for the token
                    ...
                    myJSON = JSON.stringify(myJSON);
                    $.ajax({
                        url: "//" + _4TellBoost.Service.address + "rest/Search/GetResults?clientAlias=" + _4TellBoost.SiteInfo.alias + "&token=" + _4TellBoost.UserData.session.searchToken,
                        type: "POST",
                        xhrFields: {
                            withCredentials: false
                        },
                    ...
                    //first make the call to get results, 
                    //then grab the Ids and categories and stuff we care about
                }
            }, 100);
        }
        ...
        //BEGIN THE DISPLAY PART OF SEARCH
        setInterval(function () {
            ...
            if (_4TellBoost.dataAvailable2pointO === true) {
                ...
                //if(_4TellBoost.searchResultsPage === true) {
                //alert('SRP');
                //}else {
                //if not, do the inline search stuff here
                ...
                var pLength = _4TellBoost.searchBoostResults2pointO.Results.length - 1;
    ...
    _4TellBoost.getProdImg = function (tout, itemdata) {
        ...
        var descriptionDiv = $('<div prodId="' + itemdata.productID + '" class="description">' + description + '</div>');
